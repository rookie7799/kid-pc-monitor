...
class PCTimeControl:
    ...
    def check_if_locked(self):
        """
        Returns True if screen is locked or user is not logged in,
        False if desktop is active.
        Uses X11 or Wayland to check screen state.
        """
        try:
            ...
            if os.environ.get('DISPLAY'):
                ...
                if result.returncode == 0:
                    # Could extend this to check XScreenSaver state if needed
                    return False
            
            # Check if Wayland session
            if os.environ.get('WAYLAND_DISPLAY'):
                # Wayland lock state detection is more complex
                # For now, assume unlocked if session is active
                return False
    ...
    def add_scheduled_lock(self, hour, minute):
        """Add a time when the screen should be locked"""
    ...
    def show_notification(self, message, title="PC Time Control"):
        """Display a notification using notify-send (Linux)"""
        try:
            subprocess.run(
            ...
            print(f"[{dt.now():%H:%M:%S}] Could not show notification: {e}")

    def lock_screen(self):
        """Lock the screen on Linux"""
        try:
            self.is_locked = True
            # Try multiple lock methods (i3, GNOME, KDE, xfce)
            lock_commands = [
                ['i3lock', '-c', '000000'],
            ...
            print(f"[{dt.now():%H:%M:%S}] Error locking screen: {e}")

    def shutdown_system(self, seconds=60):
        """Shutdown system with warning"""
        try:
            subprocess.run(
                ['sudo', 'shutdown', '-h', f'+{seconds//60}', f'Computer will shutdown in {seconds} seconds'],
                timeout=5,
                capture_output=True
            )
            ...
            print(f"[{dt.now():%H:%M:%S}] Error shutting down: {e}")
    ...
    def process_command(self, command):
        ...
        try:
            if command == "LOCK":
                ...
                self.pc_control.show_notification(msg)
                return "Message sent"
                ...
                return (
                    ...
                    "GET_STATUS - Check if screen is locked\n"
